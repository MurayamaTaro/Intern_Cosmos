...
def run_training(
    ...
    command = [
        ...
        # データローダー本体と、サンプラーの両方のパスを上書きする必要がある
    ...
    # CWDをワークスペースルートに変更
    print(f"Output directory (CWD): {workspace_root}")
    print(f"Log file: {log_file_path}")
    print(f"Command: {' '.join(command)}")
    print("=" * 80)

    try:
        # 実行ディレクトリ(cwd)をworkspace_rootに変更
        ...
        if process.returncode == 0:
            ...
            # Cosmosのデフォルトの出力パス構造に合わせる
            checkpoint_dir = (
                workspace_root
                / "checkpoints"
                / "posttraining"
                / "diffusion_text2world"
